...
const VendorPage =  ({params}: {params: {id: string}}) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
          ...
          let query1 = `*[ _type == "vendors" && _id == "${params.id}"][0]`
          const vendor = await  client.fetch(query1)
          console.log(vendor)
          setVendor(vendor)
      
          let query = `*[ _type == "products" && vendor._ref == "${params.id}" || _type == "beds" && vendor._ref == "${params.id}"]`
          const vendorProducts = await  client.fetch(query)
  ...
  return (
        ...
        return <div key={item.slug.current} className=''>
          <div className='product-image-with-icons w-[220px] h-[220px] sm:w-[300px] sm:h-[300px] rounded-md relative'>
            ...
            <div onMouseOver={() => setIsHovering((prev) => ({ ...prev, [item.slug.current]: true }))}
                  onMouseLeave={() => setIsHovering((prev) => ({ ...prev, [item.slug.current]: false }))} className=' flex justify-center  items-center w-[220px] h-[220px] sm:w-[300px] sm:h-[300px] bg-[#F5F5F5]'>
          ...
          )} className='bg-[#F0F2F3] hover:bg-[#029FAE] cursor-pointer rounded-lg w-[30px] h-[30px] sm:w-[40px] sm:h-[40px] flex justify-center items-center mt-2'>
          <Image className=' w-[17px] h-[17px] sm:w-[25px] sm:h-[25px] object-contain' src="/Svg/cart.svg" width={25} height={25} alt='cart' title='Add to cart' />
          </button>
          </div>
    ...
    <div className='m-5 ml-0 flex flex-wrap justify-center sm:justify-start gap-x-5  2xl:gap-x-9 gap-y-10 sm:gap-y-24 w-[101%]'>
      {products.map((item:any) => {
